...
from OWCK import GaussianProcess
...
if __name__ == '__main__':
    ...
    plot_contour_gradient(ax0, f, grad, x_lb, x_ub, title='', n_per_axis=300)
    
    f = lambda x: model.predict(x, eval_MSE=True)[1]
    grad = lambda x: model.gradient(x)[1]
        
    plot_contour_gradient(ax1, f, grad, x_lb, x_ub, title='', n_per_axis=300)
