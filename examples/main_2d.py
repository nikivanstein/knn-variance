...
# the MSE
#fig = plt.figure()
#plt.plot(range(len(x)), f(x), 'g.', label=u'$f(x) = schaffer$')
#plt.plot(range(len(X)), y, 'r.', markersize=10, label=u'Observations')
#plt.plot(range(len(x)), y_pred, 'b-', label=u'Prediction GP')
#plt.fill(np.concatenate([range(len(x)), range(len(x))[::-1]]),
#         np.concatenate([y_pred - 1.9600 * sigma,
#                        (y_pred + 1.9600 * sigma)[::-1]]),
#         alpha=.2, fc='b', ec='None', label='95% confidence interval GP')
#plt.fill(np.concatenate([range(len(x)), range(len(x))[::-1]]),
#         np.concatenate([y_pred - 1.9600 * sigma_GP,
#                        (y_pred + 1.9600 * sigma_GP)[::-1]]),
#         alpha=.2, fc='g', ec='None', label='95% confidence interval dist')
...
Z = sigma.reshape(reshape_val,reshape_val)
cset1 = plt.contourf(mesh_X, mesh_Y, Z,levels, cmap=cmap)
cset2 = plt.contour(mesh_X, mesh_Y, Z, clevels,
                colors='k',
                hold='on')
...
Z = sigma_GP.reshape(reshape_val,reshape_val)
cset1 = plt.contourf(mesh_X, mesh_Y, Z,levels, cmap=cmap)
cset2 = plt.contour(mesh_X, mesh_Y, Z, clevels,
                colors='k',
                hold='on')
...
plt.clabel(cset2, inline=1, fontsize=10)
plt.title('GP Heuristic variance')
plt.tight_layout()
plt.savefig("predvar_2d_"+benchmark_name+"_GP")
...
Z = sigma_RF_bootstrap.reshape(reshape_val,reshape_val)
cset1 = plt.contourf(mesh_X, mesh_Y, Z,levels, cmap=cmap)
cset2 = plt.contour(mesh_X, mesh_Y, Z, clevels,
                colors='k',
                hold='on')
...
plt.clabel(cset2, inline=1, fontsize=10)
plt.title('RF bootstrapping variance')
plt.subplot(2, 2, 4)
...
plt.clabel(cset2, inline=1, fontsize=10)
plt.title('RF Heuristic variance')
plt.tight_layout()
plt.savefig("predvar_2d_"+benchmark_name+"_RF")
